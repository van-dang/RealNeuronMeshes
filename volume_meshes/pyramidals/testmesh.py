...
for f in mylist:
    try:
        ...
        hmax, hmin = mesh.hmax(), mesh.hmin()
        print(hmax, hmin)
        if hmax>200 or hmin<0.001:
            print("check the mesh: ",f);
            break    
        ...
        v0 = mesh_points[0]
        for i in range(1,mesh.num_vertices()):
            eps_v=np.subtract(mesh_points[i],v0);
            eps = np.linalg.norm(eps_v)
            if eps < 1e-6:
                print("Double nodes: ")
                print([0,i],v0, mesh_points[i])
                break
    except:
        print("No file: ",f)
